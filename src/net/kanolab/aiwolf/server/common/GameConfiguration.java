...
public class GameConfiguration implements Cloneable{
	...
	private void putOptionParam(Option option, String paramString){
		...
		try {
			...
			if(Arrays.asList(Option.ENUM_OPTIONS).contains(option)) paramObject = c.getMethod("stringToParam", new Class<?>[]{String.class}).invoke("!", paramString);
			else paramObject = c.getConstructor(String.class).newInstance(paramString);
	...
	public GameConfiguration(String initFileName){
		...
		for(String line : reader.getNonSplitLines()){
			if(!line.contains(OPTION_PARAM_SEPARATOR)) continue;
		...
		for(Option option : Option.values()){
			if(!optionMap.containsKey(option.getParamClass()) || !optionMap.get(option.getParamClass()).containsKey(option)){
	...
	public Role getHumanRole(){
		...
		if(role == HumanRole.NULL) return null;
		for(Role r : Role.values()){
			if(r.toString().equalsIgnoreCase(role.toString())) return r;
		}
		return null;
	}
