...
public class NLPServerStarter extends ServerStarter {
	 ...
	 * @throws IOException
	 */
	private Socket getServerSocket(int index) throws UnknownHostException,ConnectException,NoRouteToHostException,IOException{
	...
	private void connectToPlayerServer(){
		...
		try{
			...
			for(int i=0; i<connectAgentNum; i++){
				...
				Socket socket = getServerSocket(index);
		...
		// とりあえず人狼ゲームサーバをクライアントとして使用するのは本戦のみを想定しているので、acceptClientsのように同時実行するゲームの受け付けはしない。(1ゲームのみの実行)
		waitGame(); // 何ゲームか繰り返しやる場合この実装方法だとまずい気がするけど、どうするか検討しないといけない。(1ゲームの終了をどのように把握するか分からないといけない。)
		// ↑ でも何ゲームか繰り返しやる場合ソケットを使いまわしているはずだからこの実装で良いのでは？
