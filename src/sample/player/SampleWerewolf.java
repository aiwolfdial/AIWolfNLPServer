...
public final class SampleWerewolf extends SampleBasePlayer {
	...
	public void update(GameInfo gameInfo) {
		...
		for (Judge j : divinationList) {
			...
			if (!werewolves.contains(he) && !possessedList.contains(he)
					&& ((humans.contains(target) && result == Species.WEREWOLF)
							|| (werewolves.contains(target) && result == Species.HUMAN))) {
	...
	private Judge getFakeJudge() {
		...
		if (fakeRole == Role.SEER) {
			List<Agent> candidates = aliveOthers.stream()
					.filter(a -> !myFakeJudgeMap.containsKey(a) && comingoutMap.get(a) != Role.SEER)
		...
		if (fakeRole == Role.SEER) {
			...
			if (!aliveFakeWolves.isEmpty()) {
				if (!aliveFakeWolves.contains(voteCandidate)) {
					...
					// CO後なら投票理由を付ける
					if (isCameout) {
						Content myDivination = divinedContent(me, voteCandidate,
								myFakeJudgeMap.get(voteCandidate).getResult());
			...
			for (Judge ident : identList) {
				...
				if ((myJudge != null && result != myJudge.getResult())) {
					if (isAlive(he) && !fakeWolfCandidates.contains(he)) {
						fakeWolfCandidates.add(he);
						if (isCameout) {
							Content myDivination = dayContent(me, myJudge.getDay(),
									divinedContent(me, myJudge.getTarget(), myJudge.getResult()));
			...
			for (Judge divination : divinationList) {
				...
				if ((myJudge != null && result != myJudge.getResult())) {
					if (isAlive(he) && !fakeWolfCandidates.contains(he)) {
						fakeWolfCandidates.add(he);
						if (isCameout) {
							Content myIdent = dayContent(me, myJudge.getDay(),
									identContent(me, myJudge.getTarget(), myJudge.getResult()));
	...
	void chooseFinalVoteCandidate() {
		if (!isRevote) {
			...
			if ((fakeRole == Role.SEER && aliveFakeWolves.isEmpty()) && fakeWolfCandidates.isEmpty()
